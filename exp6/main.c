...
/* depress(file_in, file_out)
 * depress reads a huff file and decode it
 * Note that I haven't made any verification in it
 * WHich means passing a file that is not generated by this program
 * could couse unintended behavior
 * 
 * Algorithm is described as follws:
 * 1. open the file and read the header
 * 2. The header contains a code_len indicating how many chars the
 *    original file has and a table
 * 3. Use the table to generate the huffman tree
 * 4. Use the huffman tree to generate the original file
 */ 
void depress(const char* name, const char* out_name) {
    FILE* fin;
    FILE* fout;
